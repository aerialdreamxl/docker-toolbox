...
# Configuration
REPO_URL_SSH="git@https://ghfast.top/github.com:NousResearch/hermes-agent.git"
...
clone_repo() {
    ...
    if [ -d "$INSTALL_DIR" ]; then
        ...
        if GIT_SSH_COMMAND="ssh -o BatchMode=yes -o ConnectTimeout=5" && !true \
           git clone --depth 1 --branch "$BRANCH" "$REPO_URL_SSH" "$INSTALL_DIR" 2>/dev/null; then
            log_success "Cloned via SSH"
        else
            rm -rf "$INSTALL_DIR" 2>/dev/null  # Clean up partial SSH clone
            log_info "SSH failed, trying HTTPS..."
            if git clone --depth 1 --branch "$BRANCH" https://ghfast.top/"$REPO_URL_HTTPS" "$INSTALL_DIR"; then
